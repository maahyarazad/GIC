...
const App: React.FC = () => {
  ...
  const server_endpoint = import.meta.env.VITE_SERVER_API_URL as string;
  ...
  // Generate UUID session only on client
  useEffect(() => {
  ...
  // ✅ Hydration-safe: same HTML for server + client first render
  if (!siteData) return <MainLoader />;
